...
function calculateDamage(skillDamageLevels, damageType, isTalent=false, isBasicSkill=false) {
    ...
    var confessionBonus = (isPVPTarget ? 0 : (parseFloat(document.getElementById('confessionBonus').value) || 0) / 100);

    for (var level = 0; level < skillDamageLevels.length; level++) {
        var skillDamage = skillDamageLevels[level];
        var totalDamage = skillDamage * (damageType === 'physical' ? (1 - targetPhysicalReduction) : (1 - targetMagicalReduction)) * (1 + talentPVEDmgBonusI + talentPVEDmgBonusII) * (1 - targetResilience) * (1 + ferocity) * (1 + castleDmg + exclusiveAttackBonus) * (1 + confessionBonus);
...
function Blame(magicalDamage){
    ...
    var percentageIncreases = [130.0, 145.0, 160.0, 180.0, 200.0];
...
function CombatSupport(physicalDamage, magicalDamage){
    ...
    var combatSupportBonusAlm = (parseFloat(document.getElementById('combatSupportBonusAlm').value) || 0) / 100;

    for (var level = 0; level < 5; level++) {
        var shield = (physicalDamage + magicalDamage) * (percentageIncreases[level] / 100) * (1 + combatSupportBonus + combatSupportBonusAlm);
...
function HaradsTeaching(physicalDamage, magicalDamage){
	...
	var haradsTeachingsBonusI = document.getElementById('haradsTeachingsBonusI').checked ? 0.025 : 0;

    var haradsTeachingsBonusAlm = document.getElementById('haradsTeachingsBonusAlm').value;
	...
	for (var level = 0; level < 4; level++) {
        if (physicalDamage > magicalDamage) {
            var damage = physicalDamage * (percentagePhysIncreases[level] / 100 + haradsTeachingsBonusI + physHaradsTeachingsBonusAlm);
        } else {
            var damage = magicalDamage * (percentageMagIncreases[level] / 100 + magHaradsTeachingsBonusAlm);
        }
        damageLevels.push(damage);
    }
...
function HaradsTeachingHeal(magicalDamage){
    ...
    var percentageMemberIncreases = [15.0, 18.0, 21.0, 25.0];
    ...
    for (var level = 0; level < 4; level++) {
        ...
        healMember = magicalDamage * (percentageMemberIncreases[level] / 100);
...
function MantraOfHealing(health){
    ...
    var percentageIncreases = [1.5, 2.5, 4.5, 6.5];
...
function ParticleOfLife(physicalDamage, magicalDamage){
	...
	for (var level = 0; level < 4; level++) {
        if (physicalDamage > magicalDamage) {
            var damage = physicalDamage * (percentagePhysIncreases[level] / 100 + particleOfLifeBonus + physParticleOfLifeBonusAlm) * (1 - relentlessBonus);
        } else {
            var damage = magicalDamage * (percentageMagIncreases[level] / 100 + particleOfLifeBonus + magParticleOfLifeBonusAlm) * (1 - relentlessBonus);
        }
        damageLevels.push(damage);
    }
