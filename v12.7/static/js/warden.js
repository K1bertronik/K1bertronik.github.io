...
function PowerfulBlow(physicalDamage){
    ...
    var powerfulBlowBonusAlm = (parseFloat(document.getElementById('powerfulBlowBonusAlm').value) || 0) / 100;

    for (var level = 0; level < 5; level++) {
        var damage = (baseValues[level] + physicalDamage * (percentageIncreases[level] / 100 + powerfulBlowBonus + powerfulBlowBonusAlm)) * (1 + relicBonus + unitedBonus);
...
function WaveOfAggression(health){
    ...
    var percentageIncreases = [6.0, 7.0, 8.0, 10.0, 12.0];
...
function PowerfulLunge(physicalDamage){
    ...
    var percentageIncreases = [160.0, 180.0, 205.0, 235.0];
    var increasePerTargetPercentages = [25.0, 30.0, 35.0, 40.0];
...
function BlockMaster(health){
    ...
    var blockMasterBonusAlm = (parseFloat(document.getElementById('blockMasterBonusAlm').value) || 0) / 100;

    for (var level = 0; level < 4; level++) {
        var baseHeal = health * (percentageIncreases[level] / 100 + blockMasterBonusI);
        var stackHeal = (health * (perStackIncreases[level] / 100 + blockMasterBonusAlm) * stacks) * (1 + blockMasterBonusIII);
