...
function Purifying(physicalDamage){
    var isBoosted = document.getElementById('purifyingComboBoost').checked;

    var damageLevels = [];
	
	var baseValues = [20, 40, 60, 80, 100];
    var percentageIncreases = [120.0, 125.0, 130.0, 135.0, 145.0];
    var boostIncreases = isBoosted ? [8.0, 10.0, 13.0, 16.0, 20.0] : [0, 0, 0, 0, 0];
    ...
    for (var level = 0; level < 5; level++) {
        var damage = baseValues[level] + physicalDamage * (percentageIncreases[level] / 100 + purifyingBonus) * (1 + boostIncreases[level] / 100) * (1 + relicBonus + unitedBonus);
...
function PurifyingDot(magicalDamage){
    ...
    for (var level = 0; level < 5; level++) {
        var damage = baseValues[level] + magicalDamage * (percentageIncreases[level] / 100 + purifyingDotBonus) * (1 + boostIncreases[level] / 100) * (1 + relicBonus + unitedBonus);
        damageLevels.push(damage);
    }
...
function HaradsBanner(magicalDamage){
    ...
    var haradsBannerBonusAlm = (parseFloat(document.getElementById('haradsBannerBonusAlm').value) || 0) / 100;

    for (var level = 0; level < 4; level++) {
        var damage = magicalDamage * (percentageIncreases[level] / 100 + haradsBannerBonus) * (1 + (percentageBoost[level] / 100 + haradsBannerBonusIII + haradsBannerBonusAlm));
        damageLevels.push(damage);
    }
...
function SunSealHeal(){
    ...
    var percentageIncreases = [12.0, 16.0, 20.0, 25.0];
...
function HaradsCall(magicalDamage){
    ...
    var percentageIncreases = [115.0, 125.0, 135.0, 150.0];
...
function RepellentStrike(physicalDamage, health){
    ...
    var percentageHealIncreases = [10.0, 13.0, 16.0, 20.0];
...
function SacredShield(health){
    ...
    var sacredShieldBonusAlm = (parseFloat(document.getElementById('sacredShieldBonusAlm').value) || 0) / 100;

    for (var level = 0; level < 4; level++) {
        var shield = health * (percentageIncreases[level] / 100) * (1 + sacredShieldBonus + sacredShieldBonusII + sacredShieldBonusAlm);
...
function PaladinsPrayer(physicalDamage, magicalDamage){
    var damageLevels = [];

    var percentagePhysIncreases = [90.0, 100.0, 110.0, 120.0];
    var percentageMagIncreases = [100.0, 110.0, 125.0, 140.0];
...
function PaladinsPrayerCrit(physicalDamage, magicalDamage){
    var damageLevels = [];

    var percentagePhysIncreases = [105.0, 120.0, 135.0, 150.0];
    var percentageMagIncreases = [120.0, 140.0, 160.0, 180.0];
