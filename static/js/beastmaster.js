...
function BeastAwakening(physicalDamage, magicalDamage, health){
    ...
    var moonTouchBoost = (parseFloat(document.getElementById('moonTouchBoost').value) || 0) / 100;
...
function ChainLightning(physicalDamage, magicalDamage){
    ...
    for (var level = 0; level < 5; level++) {
        if (physicalDamage > magicalDamage) {
            var damage = physicalDamage * (percentagePhysIncreases[level] / 100) * (1 + relicBonus);
        } else {
            var damage = magicalDamage * (percentageMagIncreases[level] / 100) * (1 + relicBonus);
        }
        damageLevels.push(damage);
    }
...
function SoulConnection(moonHealth){
    ...
    for (var level = 0; level < 4; level++) {
        var heal = moonHealth * (percentageIncreases[level] / 100);
        healLevels.push(heal);
    }
