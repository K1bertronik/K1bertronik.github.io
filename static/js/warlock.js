document.getElementById('calcForm').addEventListener('submit', function (event) {
    ...
    updateDamageValues(bloodyTributeHeal, "bloodyTributeRow");
...
function calculateDamage(skillDamageLevels, isInstantDamage=true, isTalent=false) {
    ...
    for (var level = 0; level < skillDamageLevels.length; level++) {
        ...
        var totalDamage = skillDamage * (1 - targetMagicalReduction) * (1 + talentDmgBonus + talentPVEDmgBonusI + talentPVEDmgBonusII) * (1 - targetResilience) * (1 + ferocity) * (1 + castleDmg);
...
function PoolOfDarkness(magicalDamage, energy, isPVPTarget, energyLimit){
    ...
    for (var level = 0; level < 5; level++) {
		if (acidicSteps) {
			if (isPVPTarget) {
			var damage = (magicalDamage * (percentageIncreases[level] / 100 + poolBonus) * 0.65) * (energy < energyLimit ? (1 + ((energy / 5) / 100)) : (2.6 - (900 / (energy - 300 + 900)))) * (1 + relicBonus);
			} else {
				var damage = (magicalDamage * (percentageIncreases[level] / 100 + poolBonus) * 0.65) * (energy < energyLimit ? (1 + ((energy / 5) / 100)) : (3 - (500 / (energy - 500 + 500)))) * (1 + relicBonus);
			}
		} else {
...
function BloodyTribute(magicalDamage, lostEnergy){
    var healLevels = [];
	
	var percentageEnergyIncreases = [10.0, 15.0, 25.0, 35.0];
    var percentageMagicalIncreases = [2.0, 3.0, 4.0, 5.0];
	
	var bloodyTributeBonus = document.getElementById('bloodyTributeBonus').checked ? 0.005 : 0;
	
    for (var level = 0; level < 4; level++) {
        var heal =  (lostEnergy * percentageEnergyIncreases[level] / 100) * (magicalDamage * (percentageMagicalIncreases[level] / 100 + bloodyTributeBonus));
        healLevels.push(heal);
    }
...
function ShadowSphere(magicalDamage, energy, isPVPTarget, energyLimit){
    ...
    for (var level = 0; level < 4; level++) {
		if (isPVPTarget) {
			var damage = (magicalDamage * (percentageIncreases[level] / 100 + shadowSphereBonus)) * (energy < energyLimit ? (1 + ((energy / 5) / 100)) : (2.6 - (900 / (energy - 300 + 900))));
		} else {
			var damage = (magicalDamage * (percentageIncreases[level] / 100 + shadowSphereBonus)) * (energy < energyLimit ? (1 + ((energy / 5) / 100)) : (3 - (500 / (energy - 500 + 500))));
		}
        damageLevels.push(damage);
    }
...
function StoneBody(health, energy){
    ...
    var percentageHealthIncreases = [6.0, 8.0, 10.0, 12.0];
	var percentageEnergyIncreases = [12.0, 14.0, 16.0, 20.0];
