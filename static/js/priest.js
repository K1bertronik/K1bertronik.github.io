...
function HolyShield(magicalDamage){
    ...
    for (var level = 0; level < 5; level++) {
        var shield = baseValues[level] + magicalDamage * (percentageIncreases[level] / 100 + holyShieldBonus);
...
function HealingTouch(magicalDamage){
    ...
    for (var level = 0; level < 5; level++) {
        var heal = baseValues[level] + magicalDamage * (percentageIncreases[level] / 100 + healingTouchBonus);
...
function ElusiveThreat(magicalDamage){
    ...
    var percentageDamageIncreases = [25.0, 30.0, 40.0, 65.0];
...
function MysticMark(magicalDamage){
    ...
    for (var level = 0; level < 4; level++) {
        var damage = magicalDamage * (percentageIncreases[level] / 100 + mysticMarkBonus + mysticMarkBonusII);
        damageLevels.push(damage);
    }
