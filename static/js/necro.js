document.getElementById('calcForm').addEventListener('submit', function (event) {
    ...
    var poisonShieldDamage = calculateDamage(PoisonShield(magicalDamage));
    var poisonShieldHeal = calculateHeal(PoisonShieldHeal(poisonShieldDamage));
    var infectionDamage = calculateDamage(Infection(magicalDamage, false));
    var infectionMassDamage = calculateDamage(Infection(magicalDamage, true));
    var acidRainDamage = calculateDamage(AcidRain(magicalDamage));
    var pleasureOfPain = calculateDamage(PleasureOfPain(magicalDamage));
...
function PoisonSpittle(magicalDamage){
    ...
    for (var level = 0; level < 5; level++) {
        var damage = (baseValues[level] + magicalDamage * (percentageIncreases[level] / 100 + poisonSpitBonus)) * (1 + relicBonus);
...
function BoneShield(magicalDamage){
    ...
    var percentageIncreases = [50.0, 60.0, 85.0, 100.0, 120.0];

    var boneShieldBonus = document.getElementById('boneShieldBonus').checked ? 0.10 : 0;

    for (var level = 0; level < 5; level++) {
        var shield = baseValues[level] + magicalDamage * (percentageIncreases[level] / 100 + boneShieldBonus);
...
function AncientSeal(magicalDamage){
    ...
    for (var level = 0; level < 5; level++) {
        var heal = (baseValues[level] + magicalDamage * (percentageIncreases[level] / 100 + ancientSealBonus));
...
function PoisonShield(magicalDamage){
    ...
    for (var level = 0; level < 4; level++) {
        var damage = magicalDamage * (percentageIncreases[level] / 100 + poisonShieldBonusI + poisonShieldBonusIII);
        damageLevels.push(damage);
    }
