document.getElementById('calcForm').addEventListener('submit', function (event) {
    ...
    var blowOfSpiritsDamage = calculateDamage(BlowOfSpirits(physicalDamage, magicalDamage), (physicalDamage > magicalDamage) ? 'physical' : 'magical');
...
function calculateDamage(skillDamageLevels, damageType, isInstantDamage=true, isTalent=false) {
    ...
    for (var level = 0; level < skillDamageLevels.length; level++) {
        ...
        var totalDamage = skillDamage * (damageType === 'physical' ? (1 - targetPhysicalReduction) : (1 - targetMagicalReduction)) * (1 + talentDmgBonus + talentPVEDmgBonusI + talentPVEDmgBonusII) * (1 - targetResilience) * (1 + ferocity) * (1 + castleDmg);
...
function BlowOfSpirits(physicalDamage, magicalDamage){
    ...
    var percentagePhysIncreases = [110.0, 115.0, 120.0, 125.0, 130.0];
...
function EaglesEyeMag(magicalDamage){
    ...
    for (var level = 0; level < 5; level++) {
        var damage = magicalDamage * (percentageIncreases[level] / 100 + eaglesEyeBonus) * (1 + relicBonus);
        damageLevels.push(damage);
    }
...
function EaglesEyePhys(physicalDamage){
    ...
    for (var level = 0; level < 5; level++) {
        var damage = physicalDamage * (percentageIncreases[level] / 100 + eaglesEyeBonus + eaglesEyeBonusII) * (1 + relicBonus);
        damageLevels.push(damage);
    }
...
function WolfsAlacrity(physicalDamage){
    var damageLevels = [];

    var percentageIncreases = [20.0, 25.0, 30.0, 35.0, 40.0];

    var wolfsAlacrityBonus = (parseFloat(document.getElementById('wolfsAlacrityBonus').value) || 0) / 100;

    for (var level = 0; level < 5; level++) {
        var damage = physicalDamage * (percentageIncreases[level] / 100 + wolfsAlacrityBonus) ;
        damageLevels.push(damage);
    }
